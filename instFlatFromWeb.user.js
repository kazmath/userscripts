...
// ==/UserScript==
"use strict";
const flatpak_icon_svg =
    "https://raw.githubusercontent.com/flathub-infra/website/refs/heads/main/frontend/public/img/logo/flathub-logo-mini.svg";
...
if (w.instFlatFromWeb) {
    ...
    function createButton(hidden_panel) {
        let linkPath = w.location.pathname;
        // let header_install = document.querySelectorAll("main > div > header")[0];
        // let hidden_panel = document.querySelectorAll("div[data-headlessui-portal] > button ~ div")[0];
        ...
        button_div.classList.add("items-center", "justify-center", "gap-4");
    ...
    function repeatMain() {
        let hidden_panel = document.querySelectorAll(
            "div[data-headlessui-portal] > button ~ div"
        )[0];
        if (
            document.querySelectorAll(".inst-flat-from-web-button").length ==
                0 &&
            hidden_panel != undefined
        ) {
            createButton(hidden_panel);
        } else {
            let linkPath = w.location.pathname;
            const created_button = document.querySelectorAll(
                ".inst-flat-from-web-button"
            )[0];
            if (created_button != undefined) {
                created_button.href =
                    "appstream:" +
                    linkPath.substring(linkPath.lastIndexOf("/") + 1);
            }
        }
